...
<template>
  <div v-if="link.type === 'blank'" class="w-full ">
    <div v-if="link.text === ''" class="w-full p-2"></div>
    <div v-else class="px-2 pt-5 text-sm text-gray-500">{{ link.text }}</div>
  </div>
  <div v-else class="transition transition-colors hover:bg-purple-100 px-2 py-1 rounded">
    ...
    <a v-else-if="props.link.type === 'a'" class="flex flex-row items-center gap-x-2" target="_blank"
